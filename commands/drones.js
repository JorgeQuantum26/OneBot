...
exports.run = async (client, message, args) => {
    ...
    if (!tecnologias.includes('industria_avancada')) {
        return message.channel.send('❌ Pesquise **Indústria Avançada** primeiro!\nUse: `B!pesquisa iniciar industria_avancada`');
    }
    ...
    if (!subcmd || subcmd === 'painel' || subcmd === 'status') {
        ...
        const poderTotal = (arsenal.drone_fpv || 0) * 50 + (arsenal.drone_shahed || 0) * 500 + (arsenal.drone_bayraktar || 0) * 2000;
        
        const embed = new Discord.EmbedBuilder()
            .setTitle(`🛸 Força de Drones — ${nomeFormal}`)
            .setColor('DARK_BLUE')
            .setDescription(`🏭 Fábrica: Nv. **${fabricaDrones}** (+${(fabricaDrones * 10)}% produção)`)
            .addFields({ name: '🛸 Arsenal', value: `🎮 FPV: **${arsenal.drone_fpv || 0}** (⚡50)\n` +
                `💣 Shahed: **${arsenal.drone_shahed || 0}** (⚡500)\n` +
                `🛩️ Bayraktar: **${arsenal.drone_bayraktar || 0}** (⚡2000)\n` +
                `📦 Total: **${totalDrones}** | ⚡ Poder: **${poderTotal.toLocaleString('pt-BR')}**`, inline: false });

        if (produzindo) {
            const prog = produzindo.progresso || 0;
            const tot = produzindo.total || 5;
            const barra = '🟦'.repeat(prog) + '⬜'.repeat(Math.max(0, tot - prog));
            embed.addFields({ name: '🏭 Produzindo', value: `${barra} **${Math.floor(prog/tot*100)}%**\n${DRONES[produzindo.tipo]?.nome}`, inline: false });
        }

        embed.addFields({ name: '⚡ Comandos', value: '`B!drones produzir <tipo> [qtd]` — Produzir\n' +
            '`B!drones fabrica <nível>` — Fábrica\n' +
            '`B!drones atacar <país> <tipo> <qtd>` — Atacar', inline: false });
    ...
    if (subcmd === 'tipos') {
        ...
        for (const [key, d] of Object.entries(DRONES)) {
            ...
            embed.addFields({ name: `${disponivel ? '✅' : '🔒'} ${d.emoji} ${d.nome} (\`${key}\`)`, value: `⚡ Poder: ${d.poder} | 📏 ${d.alcance}km\n💰 Custo: ${custoAjustado.toLocaleString('pt-BR')} moedas\n⏱️ ${Math.floor(d.tempo_producao / bonusFabrica)} ciclos\n📦 Silício:${d.custo_silicio||0} Alumínio:${d.custo_aluminio||0} Cobre:${d.custo_cobre||0}`, inline: false });
        }
        return message.channel.send({ embeds: [embed] });
    }
    ...
    if (subcmd === 'produzir' || subcmd === 'fabricar') {
        ...
        if (tesouro < custoTotalDinheiro) {
            const embed = new Discord.EmbedBuilder()
                ...
                .addFields({ name: '📉 Faltam', value: (custoTotalDinheiro - tesouro).toLocaleString('pt-BR'), inline: true })
        ...
        if (faltamRecursos.length > 0) {
            const embed = new Discord.EmbedBuilder()
                ...
                .addFields({ name: '📦 Faltam', value: faltamRecursos.map(r => 
                        `${r.nome}: precisa **${r.necessario.toLocaleString('pt-BR')}**, tem **${r.disponivel.toLocaleString('pt-BR')}**`
                    ).join('\n'), inline: false })
                .addFields({ name: '💡 Solução', value: 'Use `B!coletar-recursos <recurso>` para extrair.', inline: false })
                .setFooter({ text: 'Ministério da Defesa • OneBot' });
            return message.channel.send({ embeds: [embed] });
        }
    ...
    if (subcmd === 'fabrica') {
        ...
        if (tesouro < custo) return message.channel.send(`❌ Tesouro insuficiente! Custo: ${custo.toLocaleString('pt-BR')} moedas.`);
    ...
    if (subcmd === 'atacar') {
        ...
        if (!alvo || !DRONES[tipoDrone]) {
            return message.channel.send('❌ Use: `B!drones atacar <país> <tipo> [qtd]`\nTipos: `drone_fpv` `drone_shahed` `drone_bayraktar`');
        }
        ...
        if (quantidade >= 100) {
            ...
            try {
                await message.channel.awaitMessages(m => m.author.id === userId && m.content.toUpperCase() === 'CONFIRMAR', { max: 1, time: 15000, errors: ['time'] });
            } catch { return confirmMsg.edit('⏰ Cancelado.'); }
        }
        ...
        const danoAvioes = Math.floor(danoTotal * 0.10);
        ...
        if ((exInimigo.tanques || 0) > 0) db.subtract(`pais_${alvo}.exercito.tanques`, Math.min(exInimigo.tanques || 0, danoTanques));
        if ((exInimigo.avioes || 0) > 0) db.subtract(`pais_${alvo}.exercito.avioes`, Math.min(exInimigo.avioes || 0, danoAvioes));
        if ((exInimigo.navios || 0) > 0) db.subtract(`pais_${alvo}.exercito.navios`, Math.min(exInimigo.navios || 0, danoNavios));
        ...
        const noticia = {
            ...
            descricao: `**${nomeFormal}** lançou **${quantidade} ${drone.nome}** contra **${nomeAlvo}**!\n\n` +
            ...
            tipo: 'militar', impacto: 'negativo', timestamp: Date.now(), pais: nomePais
        };
        ...
        const guerra = guerras.find(g => (g.atacante === nomePais && g.defensor === alvo) || (g.defensor === nomePais && g.atacante === alvo));
        if (guerra) {
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .addFields({ name: '🦟 Modo', value: isEnxame ? `ENXAME (+${Math.floor((bonusEnxame-1)*100)}%)` : 'Normal', inline: true })
