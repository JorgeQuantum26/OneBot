...
exports.run = async (client, message, args) => {
    ...
    if (!subcmd || subcmd === 'painel' || subcmd === 'status') {
        ...
        if (fantoches.length > 0) {
            ...
            embed.addFields({ name: '💸 Tributo Total Estimado', value: `${tributoTotal.toLocaleString('pt-BR')} moedas/ciclo`, inline: true });
        } else {
        ...
        embed.addFields({ name: '⚡ Comandos de Gestão', value: '`B!fantoches necessidades <país>` — Ver necessidades\n' +
            ...
            '`B!fantoches libertar <país>` — Independência', inline: false });

        if (fantoches.length > 0 || anexados.length > 0) {
            const todosDominados = [...fantoches.map(f => f.nome), ...anexados.map(a => a.nome)];
            embed.addFields({ name: '🌍 Territórios Dominados', value: todosDominados.map(p => {
                    const d = getDadosPais(p);
                    return d ? `${d.bandeira} ${d.nomeFormal}` : p;
                }).join('\n'), inline: false });
        }

        embed.setFooter({ text: `Império de ${nomeFormal} • ${fantoches.length} fantoches, ${anexados.length} anexados` });
        embed.setTimestamp();
    ...
    if (subcmd === 'necessidades' || subcmd === 'status-pais') {
        ...
        if (comidaPorPessoa < 1) {
            necessidades.push({ tipo: '🍞 Fome', descricao: `Apenas ${comidaPorPessoa.toFixed(2)} comida/habitante`, urgencia: 'alta' });
            urgencias.push('fome');
        }

        if ((dados.infraestrutura || 0) < 2) {
            necessidades.push({ tipo: '🏗️ Infraestrutura', descricao: `Nível ${(dados.infraestrutura || 0).toFixed(1)}/5`, urgencia: 'media' });
        }

        if ((dados.inflacao || 0) > 0.10) {
            necessidades.push({ tipo: '📈 Inflação Alta', descricao: `${(dados.inflacao * 100).toFixed(1)}%`, urgencia: 'alta' });
            urgencias.push('inflacao');
        }

        if ((dados.tesouro || 0) < 100000) {
            necessidades.push({ tipo: '💰 Tesouro Crítico', descricao: `Apenas ${(dados.tesouro || 0).toLocaleString('pt-BR')} moedas`, urgencia: 'alta' });
        }
        ...
        if (tensao > 50) {
            necessidades.push({ tipo: '⚠️ Tensão Colonial', descricao: `${tensao.toFixed(1)}% - Risco de revolta!`, urgencia: 'critica' });
        ...
        const humorPopulacao = 
            satisfacao > 70 ? '😊 Satisfeita' :
            satisfacao > 40 ? '😐 Neutra' :
            satisfacao > 20 ? '😟 Descontente' :
            '🤬 Revoltada';
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .addFields({ name: '👥 População', value: `${(dados.populacao || 0).toLocaleString('pt-BR')} hab.\n` +
                `😊 Satisfação: ${satisfacao.toFixed(1)}%\n` +
                `Humor: ${humorPopulacao}`, inline: true })
            .addFields({ name: '💰 Economia', value: `Tesouro: ${(dados.tesouro || 0).toLocaleString('pt-BR')}\n` +
                `Inflação: ${((dados.inflacao || 0) * 100).toFixed(1)}%\n` +
                `Comida: ${(dados.comida || 0).toLocaleString('pt-BR')}`, inline: true })
            .addFields({ name: '⚠️ Tensão Colonial', value: '🟥'.repeat(Math.floor(tensao / 10)) + '⬜'.repeat(10 - Math.floor(tensao / 10)) + '\n' +
                `${tensao.toFixed(1)}%`, inline: true });

        if (necessidades.length > 0) {
            const textoNecessidades = necessidades.map(n => 
            ...
            embed.addFields({ name: '✅ Status', value: 'População estável, sem necessidades urgentes.', inline: false });
        }
        ...
        if (urgencias.length > 0) {
            ...
            if (urgencias.includes('tensao')) sugestoes.push('`B!fantoches reduzir-tributo ' + alvo + ' 5` ou `B!fantoches investir ' + alvo + ' estabilidade 20`');
    ...
    if (subcmd === 'recrutar') {
        ...
        if (!alvo || !tipo) return message.channel.send(`❌ Use: \`B!fantoches recrutar <país> <infantaria|tanques|avioes|navios> <qtd>\``);
        
        const fantoche = fantoches.find(f => f.nome === alvo);
        if (!fantoche) return message.channel.send(`❌ **${alvo}** não é seu fantoche!`);

        const populacao = fantoche.dados.populacao || 0;
        if (qtd > populacao * 0.01) return message.channel.send(`❌ População insuficiente! Máximo: ${Math.floor(populacao * 0.01).toLocaleString('pt-BR')}`);

        db.subtract(`pais_${alvo}.populacao`, qtd);
    ...
    if (subcmd === 'extrair') {
        ...
        if (!alvo || !['ouro', 'comida', 'madeira', 'pedra', 'dinheiro'].includes(recurso) || !qtd) {
            return message.channel.send(`❌ Use: \`B!fantoches extrair <país> <ouro/comida/madeira/pedra/dinheiro> <qtd>\``);
        }
        ...
        if (qtd > disponivel) return message.channel.send(`❌ ${alvo} só tem ${disponivel.toLocaleString('pt-BR')} de ${recurso}.`);
    ...
    if (subcmd === 'investir') {
        ...
        if (area === 'comida') { custo = Math.floor(valor * 0.5); db.subtract(`pais_${nomePais}.tesouro`, custo); db.add(`pais_${alvo}.comida`, valor); beneficio = `+${valor.toLocaleString('pt-BR')} comida`; }
        else if (area === 'infraestrutura') { custo = valor * 50000; db.subtract(`pais_${nomePais}.tesouro`, custo); db.add(`pais_${alvo}.infraestrutura`, valor); beneficio = `+${valor} infraestrutura`; }
        else if (area === 'agricultura') { custo = Math.floor(valor * 2); db.subtract(`pais_${nomePais}.tesouro`, custo); db.add(`pais_${alvo}.agricultura`, valor); beneficio = `+${valor.toLocaleString('pt-BR')} agricultura`; }
        else if (area === 'exercito') { custo = valor * 10; db.subtract(`pais_${nomePais}.tesouro`, custo); db.add(`pais_${alvo}.exercito.infantaria`, valor); beneficio = `+${valor.toLocaleString('pt-BR')} soldados`; }
        else if (area === 'estabilidade') { custo = valor * 1000; db.subtract(`pais_${nomePais}.tesouro`, custo); db.subtract(`pais_${alvo}.tensaoColonial`, Math.min(valor, (fantoche.dados.tensaoColonial || 0))); beneficio = `-${valor}% tensão`; }
        else if (area === 'populacao') { custo = Math.floor(valor * 0.1); db.subtract(`pais_${nomePais}.tesouro`, custo); db.add(`pais_${alvo}.populacao`, valor); beneficio = `+${valor.toLocaleString('pt-BR')} habitantes`; }
    ...
    if (subcmd === 'construir') {
        ...
        if (!alvo || !tipo) {
            const embed = new Discord.EmbedBuilder()
                ...
                .addFields({ name: '🏭 Industrial', value: '`industria` `usina` `mineradora` `laboratorio`', inline: true })
        ...
        if (tesouroImperial < custoTotalDinheiro) {
            return message.channel.send(`❌ Tesouro insuficiente! Custo: ${custoTotalDinheiro.toLocaleString('pt-BR')} moedas.`);
        }
        ...
        for (const [recurso, valor] of Object.entries(custo)) {
            ...
            if ((pais[recurso] || 0) < valor * qtd) {
                return message.channel.send(`❌ Falta ${recurso}! Precisa de ${(valor * qtd).toLocaleString('pt-BR')}.`);
            }
        }
        ...
        const recursosUsados = Object.entries(custo)
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .addFields({ name: '💰 Custo', value: `${custoTotalDinheiro.toLocaleString('pt-BR')} moedas`, inline: true })
    ...
    if (subcmd === 'programa' || subcmd === 'saude') {
        ...
        const programasDisponiveis = {
            ...
            saneamento: { nome: 'Saneamento Básico', emoji: '🚰', custo: 50000000, efeito: 'Reduz doenças hídricas em 80%' },
            controle_pragas: { nome: 'Controle de Pragas', emoji: '🐀', custo: 20000000, efeito: 'Reduz vetores de doenças' },
            quarentena: { nome: 'Quarentena Obrigatória', emoji: '🔒', custo: 10000000, efeito: 'Reduz contágio em 50%' },
        };
    ...
    if (subcmd === 'imposto' || subcmd === 'taxa') {
        ...
        const taxaAntiga = ((fantoche.dados.taxaImposto || 0.10) * 100).toFixed(0);
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .addFields({ name: '📊 Impacto', value: novaTaxa > 20 ? '⚠️ Imposto alto pode gerar insatisfação' : 
                novaTaxa > 10 ? '✅ Taxa moderada' : '🟢 Taxa baixa', inline: false })
    ...
    if (subcmd === 'pesquisa' || subcmd === 'tecnologia') {
        ...
        const TECNOLOGIAS_DISPONIVEIS = {
            industria_avancada: { nome: 'Indústria Avançada', emoji: '🏭', custo: 50000000, efeito: '+10% produtividade' },
            revolucao_verde: { nome: 'Revolução Verde', emoji: '🌾', custo: 40000000, efeito: '+15% comida' },
            militar_basico: { nome: 'Doutrina Militar Básica', emoji: '⚔️', custo: 60000000, efeito: '+5% poder militar' },
        ...
        if (tecId === 'industria_avancada') db.add(`pais_${alvo}.produtividade`, 0.10);
    ...
    if (subcmd === 'suprimir') {
        ...
        db.subtract(`pais_${alvo}.populacao`, Math.floor((fantoche.dados.populacao || 0) * 0.02));
    ...
    if (subcmd === 'libertar') {
        ...
        const noticia = {
            ...
            tipo: 'comercio', impacto: 'positivo', timestamp: Date.now(), pais: nomePais
        };
    ...
    if (subcmd === 'enviar-tropas' || subcmd === 'reforcar' || subcmd === 'reforcos') {
        ...
        if (!alvo) {
            return message.channel.send('❌ Use: `B!fantoches reforcar <país> <tipo1> <qtd1> [tipo2] [qtd2] ...`\n\n📋 **Tipos:** `infantaria` `tanques` `avioes` `navios` `drones`');
        }
        ...
        if (!args[2]) {
            ...
            const embed = new Discord.EmbedBuilder()
                ...
                .setColor('#3498db')
                .setDescription(`Envie tropas e equipamentos para **${alvo}**.\nUse: \`B!fantoches reforcar ${alvo} <tipo> <qtd> [tipo2] [qtd2]...\``)
                .addFields({ name: '🟢 Suas Tropas Disponíveis', value: `🪖 Infantaria: **${(exMeu.infantaria || 0).toLocaleString('pt-BR')}**\n` +
                    ...
                    `🛸 Drones: **${dronesMeu}**`, inline: true })
                .addFields({ name: '🔴 Tropas do Fantoche', value: `🪖 Infantaria: **${(exAlvo.infantaria || 0).toLocaleString('pt-BR')}**\n` +
        ...
        for (const envio of envios) {
            if (envio.tipo === 'drones') {
                ...
                for (const t of ['drone_fpv', 'drone_shahed', 'drone_bayraktar', 'drone_swarm']) {
                    ...
                    if (tirar > 0) { db.subtract(`pais_${nomePais}.arsenal_drones.${t}`, tirar); restantes -= tirar; }
                }
        ...
        const noticia = {
            ...
            tipo: 'militar', impacto: 'positivo', timestamp: Date.now(), pais: nomePais
        };
        const engine = client.paisEngine;
        if (engine) {
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .addFields({ name: '📦 Reforços', value: envios.map(e => `• ${e.tipo}: **${e.qtd.toLocaleString('pt-BR')}**`).join('\n'), inline: false })
