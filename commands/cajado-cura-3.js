...
let vida = await db.fetch(`hp_${user.id}`);
  ...
  .setDescription(`${message.author}, Você restaurou **100** de Vida Curando seus ferimentos em Massa! Porém você perdeu 5 cargas, Seu Cajado da Cura desapareceu num lampejo de luz!`)
  .setFooter({ text: `© RPG OneBot` })
  .setTimestamp()
  message.channel.send({ embeds: [carga1] });
  

  db.set(`hp_${user.id}`, 100);
  db.add(`carga_${user.id}`, 55);
  db.subtract(`cajado_${user.id}`, 1);
}
