...
const embed1 = new Discord.EmbedBuilder()
  ...
  if(ban >= 1) return message.channel.send({ embeds: [embed1] })
    ...
    let coins = await db.fetch(`rpgcoins_${user.id}`)
      ...
      if(coins <= 3310) return message.channel.send(`<a:nao:868232161289986128>**|**${message.author}, Você não possuí RPG coins o Suficiente! \`${coins}/3310\` Para realizar esta compra! `);

    let força = await db.fetch(`peso1_${user.id}`);
    if(força >= 1) return message.channel.send(`<a:nao:868232161289986128>**|**${message.author}, Você já realizou esa compra e já tem Um **Peso 50kg**. Não pode comprar novamente.`);

    
    const embed = new Discord.EmbedBuilder()
    .setColor('Random')
    .setTitle(`**Compra Realizada com Sucesso!**`)
    .setDescription(`${message.author}, Você comprou um **Peso** de 50kg Por **3310** Coins com Sucesso!\n\nUse \`B!treinar-50kg\``)
    .setFooter({ text: `© RPG OneBot` })
    .setTimestamp()

    message.channel.send({ embeds: [embed] })

    db.add(`peso1_${user.id}`, 1);
    db.subtract(`rpgcoins_${user.id}`, 3310);
  }
}